    ...
    navbarPage("Median Income Examinations by Major and Gender",
        ...
        tabPanel("Median Income Gap By Major and State", # 2nd graph panel
                ...
                sidebarLayout(
                  ...
                  mainPanel(
                    plotOutput("wage_gap_plot", height = "600px", width = "100%") # controls graph size automatically
                  )
                )
        )
    )
...
server = function(input, output, session) {
    ...
    output$bargraph = renderPlot({
        ...
        ggplot(cleaned_data(), aes(x = reorder(Major, Income_Difference), y = Income_Difference)) +
            ...
            labs(title = "How Much More/Less Each Major is Recieving than Livable Wage",
                 caption = 
                 "This graph shows how different each major's income is from the
                 livable wage of a particular state. This was visualizes if each
                 major can live confortably given the wage that they are 
                 recieving.The data about each major's income was taken from the
                 United States Census Bureau where as the data about the livable
                 wages per state come from the World Population Review.") +
    ...
    ggplot(gap_long, aes(x = reorder(Major, WageGap), y = WageGap)) +
      ...
      labs(
        ...
        caption = 
          paste("This chart shows the wage gap between genders across various states and majors.",
        "Females tend to earn less than their male counterparts in nearly every state and major,",
        "highlighting a broader trend of gender inequity in the U.S.",
        "Source: United States Census Bureau.", sep = "\n"),
        x = "Major",
